...
class UiTestLib(object):
    ...
    def swipe(self, sx, sy, ex, ey, steps=20):
        ...
        """
        return self.d.swipe(int(sx),int(sy), int(ex),int(ey), int(steps))

    def drag(self, sx, sy, ex, ey, steps=20):
        """
        Drag from (sx,sy) to (ex,ey)
        """
        return self.d.drag(int(sx),int(sy), int(ex),int(ey), int(steps))
    ...
    def take_screenshot(self, scale=None, quality=None):
        ...
        logger.info('\n<a href="%s.png">%s</a><br><img src="%s.png">' % (st, st, st), html=True)
    ...
    def wait_for_logcat_line(self, search, timeout=30):
        """ Wait for search string exists in given timeout, return total line if have,
        otherwise will return None
        search string is your want to search, log can be a regular expression
        time in seconds, default value is 30 seconds

        Example:
        | Wait For Logcat Line | .*ActivityManager.*cmp=com.sonyericsson.album/com.sonyericsson.album.Main.* |
        """
        start = time.time()
        while (time.time() - start) < int(timeout):
            log_proc = self.adb.cmd("logcat -d -v time")
            lines = log_proc.stdout.readlines()
            for line in lines:
                rlt = re.search(search, line.rstrip())
                if rlt is not None:
                    return rlt.group(0)
                    self._result = True
            time.sleep(2)  #sleep 2s to wait
    ...
    def end_phone_call(self):
        ...
        """
        if self.get_call_status() == "2":
            self.press_key("6")
        return self.get_call_status() == "0"
...
if __name__ == "__main__":
    ...
    p = UiTestLib('d34cba16')
    p1 = UiTestLib('d343ba16')
    #print p.get_uptime()

...
class UiKpiTest(UiTestLib):
    ...
    ####@@@@@@@@@@@@########################
    #   For test Pepper and Meta Functions #
    ########################################
    def get_speech_recognized_result(self, timeout=10):
        """
        Wait speech recognized result return, return None is not available
        X = p.get_speech_recognized_result()
        if X != None: print X[0]

        Return text, seq
        """
        #ret = '03-01 11:34:54.719 26045  8292 D HS/WSRTCClient: run: sendInfo {"type":"qa","seq":1088,"data":{"question":{"lang":"CH","text":"明天天气怎么样"}},"id":"info","sid":"4154eba0-1c3c-11e8-9b9e-e378f8205352","from":"huaping"}'
        rlt =  self.wait_for_logcat_line(".*WSRTCClient.*seq.*question.*text.*from.*", timeout)
        #rlt = re.search(".*WSRTCClient.*seq.*question.*text.*from.*", ret)
        if rlt != None:
            rltJson = rlt.split('sendInfo')[-1]
            d = json.loads(rltJson)
            # return text, seq
            return  d['data']['question']['text'], d['seq']
        else:
            return None

    def check_speech_recognized(self, question, contain_question, lang = 'zh-cn'):
        """
        Check Speech recognized
        question: to speak
        contain_question - contain string in question
        """
        utils = Utils()
        self.clear_logcat()
        utils.play_sound_on_PC_from_text(question, lang=lang)
        A = self.get_speech_recognized_result(timeout=10)
        if A != None:
            print("Question recognized result:"+A[0])
            if A[0].find(contain_question) != -1:
                return True
        return False


    def get_anwser_from_hari(self, timeout = 15):
        """
        return text,seq
        #ret = '03-01 13:30:25.948 D/HS/WSChannelClient(26045): WSS->C:
        {"type":"qa","seq":2449,"from":"huaping@cloudminds.com","src":"HI",
        "data":{"question":{"lang":"CH","questionId":"a4d23810-1d11-11e8-9b9e-e378f8205352","simQaFlag":false,"text":"你好小达"},
        "answer":[{"text":"您好","score":1}],"uuid":"b4002a93-b926-9ed7-d3fd-e86a6b2bf3f1"},"id":"info","sid":"4154eba0-1c3c-11e8-9b9e-e378f8205352","ver":"1.0"}'
        """
        rlt = self.wait_for_logcat_line(".*WSChannelClient.*:\s+WSS->C:\s+.*answer.*", timeout)
        #rlt = re.search(".*WSChannelClient.*:\s+WSS->C:\s+.*answer.*", ret)

        if rlt != None:
            rltJson = rlt.split('WSS->C:')[1]
            d = json.loads(rltJson)
            print d['data']["answer"][0]["text"]
            print d['seq']
            return d['data']["answer"][0]["text"], d['seq']
        else:
            return None

    def check_question_and_anwser(self, question, anwser, lang='zh-cn'):
        """
        Return True is have right anwser
        """
        utils = Utils()
        self.clear_logcat()
        utils.play_sound_on_PC_from_text(question, lang=lang)
        ...
        if A != None:
            print("Question recognized result:"+A[0])
            B = self.get_anwser_from_hari(timeout=5)
            if B != None:
                print(A[0])
                if A[1] != B[1]:
                    print('Seqence is not same, please make your evironment quiet')
                    return False
                if B[0].find(anwser) != -1:
                    print( B[0])
                    return True
        return False
...
if __name__ == "__main__":
    ...
    p = UiKpiTest('d0caeb64')
    p.clear_logcat()
    print p.check_speech_recognized(u"北京天气怎么样", u"天气")
    #print p.check_question_and_anwser(u"北京天气怎么样", u"气温")
    #print p.get_anwser_from_hari()
#    X = p.get_speech_recognized_result()
#    if X != None: print X[0], X[1]
